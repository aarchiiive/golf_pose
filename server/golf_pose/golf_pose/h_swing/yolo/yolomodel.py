...
class YOLOModel:
    ...
    def forward(self, video_path):
        ...
        for frame, r in enumerate(self.results):
    ...
    #       self.image = cv2.resize(self.image,(860,640))
            ...
            self.zero_found = any(0 in sublist for sublist in self.keypoint)
            if self.zero_found:
                continue
            else:
                ...
                self._get_lines(frame, self.keypoint)
                self._save_keypoints(frame)
    ...
    def make_sorted_events(self):
        for frame, keypoint in enumerate(self.keypoints):
            ...
            print(max_right, wrist_keypoint[0])
    ...
    def _save_keypoints(self, frame):
